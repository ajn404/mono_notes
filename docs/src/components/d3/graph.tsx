...
const Circle = () => {
    ...
    useEffect(() => {
        ...
        const simulation = d3.forceSimulation(nodes as SimulationNodeDatum[])
            .force("link", d3.forceLink(links as SimulationLinkDatum<any>[]).id((d: any) => d.id).distance(300).strength(0.5))
            .force("charge", d3.forceManyBody().strength(-50))
            .force("x", d3.forceX())
            .force("y", d3.forceY());
        svgElement
            .attr("viewBox", [-width / 2, -height / 2, width, height])
            .attr("style", `width:${100}vw;max-width: 100%; height: calc( 100vh - 100px);pointer-events:none;`)
        ...
        const text = svgElement.append("g")
            .attr("class", "text")
            .selectAll("text")
            .data(nodes)
            .enter().append("text")
